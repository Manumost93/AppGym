...
- **CI/CD**: GitHub Actions compila y testea backend+frontend en cada push; en `main` ademas construye y publica las 5 imagenes del backend en GHCR (`ghcr.io/<usuario>/appgym-*`) **multi-arquitectura** (`linux/amd64` + `linux/arm64`), listas para desplegar sin compilar nada en el servidor de produccion. El Dockerfile compila el jar una sola vez de forma nativa (`--platform=$BUILDPLATFORM` en el stage de build, ya que el bytecode Java es independiente de la arquitectura) y solo repite, por cada arquitectura de destino, el paso barato de copiarlo a la imagen runtime — evita emulacion QEMU en el paso pesado.
- **Despliegue**: backend en VPS (Hetzner de pago o, alternativa gratuita, Oracle Cloud Free Tier con instancias ARM Ampere — de ahi el build multi-arquitectura) con `docker-compose.prod.yml` + Traefik (Let's Encrypt automatico, solo `api-gateway` es publico; ver `docs/deployment.md`). Sin dominio propio, se usa `nip.io` como DNS gratuito apuntando a la IP del VPS, con HTTPS real igualmente. Frontend Angular en Vercel. Se prefirio a un PaaS tipo Render por coste y porque demuestra mejor el dominio de Docker/orquestacion.
